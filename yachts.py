...
def sun(xc, yc, a, b): # центр, радиус, длина луча + радиус
    brushColor('yellow')
    penColor('yellow')
    penSize(0)
    for i in range(40):
        phi1 = i*pi/20
        phi2 = (i+1)*pi/20
        polygon ([(xc, yc), (xc + a*cos(phi1), yc + sin(phi1)), (xc + b*cos(phi1+pi/40), yc + b*sin(phi1+pi/40)), (xc + a*cos(phi2), yc + a*sin(phi2))])
    for i in range(40):
        phi1 = i*pi/20
        phi2 = (i+1)*pi/20
        polygon ([(xc, yc), (xc - a*cos(phi1), yc + sin(phi1)), (xc - b*cos(phi1+pi/40), yc + b*sin(phi1+pi/40)), (xc - a*cos(phi2), yc + a*sin(phi2))])
